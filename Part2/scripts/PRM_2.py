...
def quaterniomn_to_rotation_matrix(quaternion):
    w = quaternion[0]
    x = quaternion[1]
    y = quaternion[2]
    z = quaternion[3]
...
class PRM:
    ...
    def if_path_collisionfree(self, q, q_prime):
        ...
        for i in range(n_step):
            q = q + dq
            if pqp_client(q[0], quaterniomn_to_rotation_matrix(q[1])) == True:
                return False
        return True
...
if __name__ == '__main__':
    ...
    while True:
        start = sample_config()
        result = pqp_client(start[0], quaterniomn_to_rotation_matrix(start[1]))
        if result == False:
            break

    while True:
        goal = sample_config()
        result = pqp_client(goal[0], quaterniomn_to_rotation_matrix(goal[1]))
        if result == False:
            break
