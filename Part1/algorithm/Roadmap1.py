...
class RoadMap:
    ...
    def __init__(self):
        ...
        self.can_be_visited = {}  # for each configuration q, all the points that q can already fand a path to visit
    ...
    def build_roadmap(self):
        for i in range(NUM_SAMPLE):
            while True:
                ...
                if result == False:
                    break
            self.can_be_visited[(tuple(q[0]), tuple(q[1]))] = [q]
            ...
            for i in range(len(N_indices)-1):
                ...
                if self.if_path_collisionfree(q, q_prime) and q_prime not in self.can_be_visited[(tuple(q[0]), tuple(q[1]))]:
                    self.E.append((q, q_prime))
                    # add all visited list of q_prime to q
                    for j in range(len(self.can_be_visited[(tuple(q_prime[0]),tuple(q_prime[1]))])):
                        self.can_be_visited[(tuple(q[0]), tuple(q[1]))].append(self.can_be_visited[(tuple(q_prime[0]), tuple(q_prime[1]))][j])
